...
export async function deleteAd(adId, userId, isAdmin = false) {
    if (isFirebaseLive && firebaseDb && !adId.startsWith('mock_ad_')) {
        try {
            ...
            if (docSnap.exists() && (docSnap.data().userId === userId || isAdmin)) {
        ...
        const adIndex = ads.findIndex(a => a.id === adId);
        if (adIndex !== -1) {
            if (ads[adIndex].userId === userId || isAdmin) {
...
export async function updateAd(adId, adData, userId, isAdmin = false) {
    if (isFirebaseLive && firebaseDb && !adId.startsWith('mock_ad_')) {
        try {
            ...
            if (docSnap.exists() && (docSnap.data().userId === userId || isAdmin)) {
                ...
                return { id: adId, ...docSnap.data(), ...adData };
            }
            throw new Error('Unauthorized update or ad not found');
        } catch (error) {
            console.error('Firestore update ad error:', error);
            throw error;
        }
    } else {
        const ads = getMockAds();
