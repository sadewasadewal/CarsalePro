...
import { fetchAds, addAd, incrementAdViews, deleteAd, updateAdStatus, updateAd } from './db.js';
...
let currentPage = 1;
let totalPages = 1;
let filteredAds = []; // Currently filtered/sorted ads
...
const elements = {
    ...
    btnLogout: document.getElementById('nav-logout-btn'),
    ...
    btnDeleteModal: document.getElementById('btn-delete-modal'),
    adminPendingText: document.getElementById('admin-pending-text'),
    adminPendingActions: document.getElementById('admin-pending-actions'),
    editModal: document.getElementById('edit-modal'),
    btnCloseEditModal: document.getElementById('btn-close-edit-modal'),
    adEditForm: document.getElementById('ad-edit-form'),
    editAdTitle: document.getElementById('edit-ad-title'),
    editAdMake: document.getElementById('edit-ad-make'),
    editAdModel: document.getElementById('edit-ad-model'),
    editAdYear: document.getElementById('edit-ad-year'),
    editAdPrice: document.getElementById('edit-ad-price'),
    editAdLocation: document.getElementById('edit-ad-location'),
    editAdNegotiable: document.getElementById('edit-ad-negotiable'),
...
document.addEventListener('DOMContentLoaded', async () => {
    ...
    // 1. Init static Lucide icons immediately
    if (window.lucide) {
        lucide.createIcons();
    }

    // 2. Initialize Firebase or fallback
    showLoader(true);
...
function handleHashRouting() {
    ...
    if (targetTab) {
        ...
        // Toggle footer visibility (hide on sign-in/register page)
        ...
        if (tabName === 'home') {
            // Home is now the landing page - no ads to load
...
function setupEventHandlers() {
    ...
    // Hero Get Started button -> go to browse
    ...
    if (mobileMenuBtn && navLinksContainer) {
        mobileMenuBtn.addEventListener('click', (e) => {
            ...
            if (icon) {
                const isShowing = navLinksContainer.classList.contains('show');
    ...
    elements.searchInput.addEventListener('input', debounce(() => { currentPage = 1; loadAdsGrid(); }, 300));
    ...
    elements.filterLocation.addEventListener('change', () => { currentPage = 1; loadAdsGrid(); });
    
    // Sort select handler
    if (elements.sortSelect) {
        elements.sortSelect.addEventListener('change', () => { currentPage = 1; loadAdsGrid(); });
    }
    ...
    if (elements.btnPrevPage) {
        elements.btnPrevPage.addEventListener('click', () => {
            if (currentPage > 1) {
                currentPage--;
                renderCurrentPage();
                document.querySelector('.fleet-header').scrollIntoView({ behavior: 'smooth', block: 'start' });
            }
        });
    }
    ...
    // Admin Edit and Delete modal listeners
    if (elements.btnDeleteModal) {
        elements.btnDeleteModal.addEventListener('click', handleAdminDeleteAd);
    }
    if (elements.btnEditModal) {
        elements.btnEditModal.addEventListener('click', handleOpenEditAdModal);
    }
    if (elements.btnCloseEditModal) {
        elements.btnCloseEditModal.addEventListener('click', () => hideModal(elements.editModal));
    }
    if (elements.adEditForm) {
...
function updateAuthUI(user) {
    ...
    if (user) {
        ...
        // Populate profile info: avatar image or first letter fallback
        if (user.photoURL) {
            ...
            if (userAvatarLetter) {
                ...
                userAvatarLetter.innerText = displayName.charAt(0).toUpperCase();
                userAvatarLetter.classList.remove('hidden');
            }
        }
        ...
        const adSellerName = document.getElementById('ad-seller-name');
        const adSellerEmail = document.getElementById('ad-seller-email');
        if (adSellerName) adSellerName.value = user.displayName || '';
        if (adSellerEmail) adSellerEmail.value = user.email || '';
        ...
        if (elements.btnLogout) elements.btnLogout.classList.add('hidden');
...
async function loadAdsGrid() {
    ...
    try {
        ...
        // Sort the ads
        sortAds(currentAds);
        filteredAds = currentAds;
        currentPage = 1;
        totalPages = Math.ceil(filteredAds.length / ADS_PER_PAGE);
        renderCurrentPage();
...
function sortAds(ads) {
    const sortVal = elements.sortSelect ? elements.sortSelect.value : 'latest';
    if (sortVal === 'latest') {
        ads.sort((a, b) => (b.createdAt || 0) - (a.createdAt || 0));
    } else if (sortVal === 'oldest') {
        ads.sort((a, b) => (a.createdAt || 0) - (b.createdAt || 0));
...
function renderCurrentPage() {
    ...
    renderAds(pageAds, filteredAds.length);
    renderPagination();
}

function renderPagination() {
    if (!elements.paginationControls || !elements.pageNumbers) return;
    ...
    range.forEach(item => {
        if (item === '...') {
            ...
            btn.className = `page-num-btn${item === currentPage ? ' active' : ''}`;
...
function renderAds(ads, totalCount) {
    elements.adsContainer.innerHTML = '';
    const count = totalCount !== undefined ? totalCount : ads.length;
    elements.resultsCount.innerText = `Showing ${count} listing${count === 1 ? '' : 's'}`;
    
    if (count === 0) {
        elements.emptyState.classList.remove('hidden');
        if (elements.paginationControls) elements.paginationControls.classList.add('hidden');
        return;
    }
    ...
    ads.forEach(ad => {
        ...
        card.innerHTML = `
            <div class="card-image-box">
                <img src="${coverImg}" alt="${escapeHTML(ad.title)}" class="card-image" loading="lazy">
                <div class="card-badges">
                    <span class="badge badge-year">${ad.year}</span>
                    <span class="badge badge-location"><i data-lucide="map-pin"></i> ${escapeHTML(ad.location)}</span>
                </div>
                <div class="card-views-badge">
                    <i data-lucide="eye"></i> <span>${ad.views || 0} views</span>
                </div>
            </div>
            <div class="card-body">
    ...
    // Re-trigger lucide + card entrance animations
    lucide.createIcons();
    // Stagger card animations by re-adding animation class
    const cards = elements.adsContainer.querySelectorAll('.ad-card');
...
async function openAdDetails(ad) {
    ...
    // Coerce views count safely to number
    const initialViews = (typeof ad.views === 'number') ? ad.views : (ad.views === true ? 1 : parseInt(ad.views) || 0);
    elements.modalViews.innerText = initialViews;
    ...
    if (modalAdminActions) {
        if (isAdminUser(currentUser)) {
            ...
            // If under review, show pending text and approve/reject actions
            if (ad.status === 'under_review') {
                if (elements.adminPendingText) elements.adminPendingText.classList.remove('hidden');
                if (elements.adminPendingActions) elements.adminPendingActions.classList.remove('hidden');
            } else {
                if (elements.adminPendingText) elements.adminPendingText.classList.add('hidden');
                if (elements.adminPendingActions) elements.adminPendingActions.classList.add('hidden');
            }
        } else {
    ...
    // Ensure all dynamic icons in modal are initialized
    if (window.lucide) {
        window.lucide.createIcons();
    }
    ...
    try {
        ...
        if (updatedViews !== null && updatedViews !== false) {
            const currentViewsNum = (typeof ad.views === 'number') ? ad.views : (ad.views === true ? 1 : parseInt(ad.views) || 0);
            const newCount = typeof updatedViews === 'number' ? updatedViews : currentViewsNum + 1;
            elements.modalViews.innerText = newCount;
            ad.views = newCount;
        }
    } catch (e) {
...
function setupModalGallery(images) {
    ...
    // Update counter
    if (elements.imgCounter) elements.imgCounter.textContent = `1 / ${imgList.length}`;
    ...
    if (imgList.length <= 1) {
        ...
        imgList.forEach((src, idx) => {
            ...
            thumb.addEventListener('click', () => setDetailImage(idx));
...
function setDetailImage(index) {
    ...
    // Fade transition
    elements.modalMainImage.style.opacity = '0';
    setTimeout(() => {
        elements.modalMainImage.src = images[index];
        elements.modalMainImage.style.opacity = '1';
    }, 150);
    
    // Update counter
    if (elements.imgCounter) elements.imgCounter.textContent = `${index + 1} / ${images.length}`;
...
async function handleAdminDeleteAd() {
    ...
    if (confirm(`Are you sure you want to permanently delete "${currentAdDetail.title}"? This cannot be undone.`)) {
        showLoader(true);
        try {
            await deleteAd(currentAdDetail.id, currentUser.uid, true);
            showToast('Ad deleted successfully!', 'success');
...
function handleOpenEditAdModal() {
    if (!currentAdDetail) return;
    
    // Pre-populate fields
    elements.editAdTitle.value = currentAdDetail.title;
    elements.editAdMake.value = currentAdDetail.make;
    elements.editAdModel.value = currentAdDetail.model;
    elements.editAdYear.value = currentAdDetail.year;
    elements.editAdPrice.value = currentAdDetail.price;
    elements.editAdLocation.value = currentAdDetail.location || 'Colombo';
    elements.editAdNegotiable.checked = !!currentAdDetail.negotiable;
    elements.editAdDescription.value = currentAdDetail.description;
    elements.editAdSellerName.value = currentAdDetail.sellerName;
    elements.editAdSellerPhone.value = currentAdDetail.sellerPhone;
    elements.editAdWhatsappLink.checked = currentAdDetail.whatsappLinkEnabled !== false;
...
async function handleEditAdSubmit(e) {
    ...
    const adData = {
        title: elements.editAdTitle.value.trim(),
        make: elements.editAdMake.value.trim(),
        model: elements.editAdModel.value.trim(),
        year: parseInt(elements.editAdYear.value),
        price: parseFloat(elements.editAdPrice.value),
        negotiable: elements.editAdNegotiable.checked,
        location: elements.editAdLocation.value,
        description: elements.editAdDescription.value.trim(),
        sellerName: elements.editAdSellerName.value.trim(),
        sellerPhone: elements.editAdSellerPhone.value.trim(),
        whatsappLinkEnabled: elements.editAdWhatsappLink.checked
    };
    ...
    showLoader(true);
    try {
        await updateAd(currentAdDetail.id, adData, currentUser.uid, true);
